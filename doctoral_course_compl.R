...
basis_registration <- dbGetQuery(con, statement = read_file('SQL/course_compl/doctoral_course_compl.sql'))


ssi_totals <-basis_registration |> 
...
# let's calculate total number of students per year
...
course_data <- basis_registration |>
  group_by(time_academic_yr, class_course, credit_hours) |>
...
final_calcs <- results |> left_join(ssi_totals, by = c('fiscal_year' = 'ssi_payout_y1')) |>
   ungroup() |> 
   mutate(ssi_per_course_per_cr_hours = ssi_y1_payout_total_amount * credit_hours_weight/cumul_total_cr_hours) |> 
   select(class_course,fiscal_year,ssi_per_course_per_cr_hours)


# Gathering & recalculating --------------------------------------------------------------

temp_1 <- basis_registration |> select(- ssi_y1_payout_total_amount) |> 
  left_join(final_calcs, by = c( 'ssi_payout_y1' = 'fiscal_year', 'class_course' = 'class_course')) |> 
  rename(ssi_y1_payout_per_cr_hour = ssi_per_course_per_cr_hours)

temp_1 <- temp_1 |> 
  left_join(final_calcs, by = c( 'ssi_payout_y2' = 'fiscal_year', 'class_course' = 'class_course')) |> 
  rename(ssi_y2_payout_per_cr_hour = ssi_per_course_per_cr_hours)


temp_1 <- temp_1 |> 
  left_join(final_calcs, by = c('ssi_payout_y3' = 'fiscal_year', 'class_course' = 'class_course')) |> 
...
# re-coding some completes that are in fact incompletes
...
# some checks
# final_ssi_courses |>
#   filter(complete_flag == 'complete') |>
#   group_by(time_academic_yr) |>
#   summarise(
#     min_y1 = min(ssi_y1_payout_total_ssi),
#     min_y2 = min(ssi_y2_payout_total_ssi),
#     min_y3 = min(ssi_y3_payout_total_ssi)
#   )
...
csv_file <- 'data_final/ssi_doctoral_subsidy.csv'

readr::write_csv(final_ssi_courses,csv_file,na = '')

# will append these records to the existing table.
