...
basis_ssi <- dbGetQuery(con, statement = read_file('SQL/degree_completions_final.sql'))
...
temp <- basis_ssi |> 
  left_join(degrees_cohort_totals,by = c('matchid' = 'matchid','ssi_payout_y1' = 'fiscal_year'))|>
  rename(ssi_payout_y1_total_degree = total_degrees)

temp <- temp |> 
  left_join(degrees_cohort_totals,by = c('matchid' = 'matchid','ssi_payout_y2' = 'fiscal_year')) |>
  rename(ssi_payout_y2_total_degree = total_degrees)
  
temp <- temp |> 
  left_join(degrees_cohort_totals,by = c('matchid' = 'matchid','ssi_payout_y3' = 'fiscal_year')) |> 
...
ssi_totals <- temp |>
  distinct(matchid,ssi_payout_y1,ssi_y1_payout_total_amount) |> 
  rename(fiscal_year =  ssi_payout_y1, ssi_amount = ssi_y1_payout_total_amount) 


temp <- temp |> left_join(ssi_totals, by =c('matchid' = 'matchid', 'ssi_payout_y2' = 'fiscal_year')) |> 
  rename('ssi_y2_payout_total_amount' = 'ssi_amount')

temp <- temp |> left_join(ssi_totals, by =c('matchid' = 'matchid', 'ssi_payout_y3' = 'fiscal_year')) |> 
...
# for some matchids we might have degrees but we don't have SSI dollars
...
csv_file <- 'data_final/ssi_student_degree_subsidy.csv'

dbCreateTable(con, Id(schema = schema, table = table), final_ssi_degrees)
